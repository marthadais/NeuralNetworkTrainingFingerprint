...
def plot_results(file):
    with open(f'./data/dist_matrix/{file}.pickle', "rb") as f:
        ddata = pickle.load(f)
        plt.figure(figsize=(10, 10))
        plt.imshow(ddata)
        plt.xlabel("Iteration")
        ...
        plt.title("Distance matrix for transition data: \n" + file)
        plt.colorbar()
        plt.savefig(('./data/figures/transition_' + file + '.png'), bbox_inches='tight')
        plt.close()

    with open(f'./data/snapshots_info/{file}.pickle', "rb") as f:
        sdata = pickle.load(f)
        preds = []
        acc = []
        n = len(sdata)
        ...
        plt.plot(acc)
        plt.tight_layout()
        plt.savefig(('./data/figures/predictions_' + file + '.png'), bbox_inches='tight')
        plt.close()
